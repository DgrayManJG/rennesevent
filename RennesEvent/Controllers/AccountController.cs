    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Identifiant);
                    // var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Identifiant, code = code }, protocol: Request.Url.Scheme);
                    // await UserManager.SendEmailAsync(user.Identifiant, "Confirmez votre compte", "Confirmez votre compte en cliquant <a href=\"" + callbackUrl + "\">ici</a>");
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // Pour plus d'informations sur l'activation de la confirmation du compte et la réinitialisation du mot de passe, consultez http://go.microsoft.com/fwlink/?LinkID=320771
                // Envoyer un message électronique avec ce lien
                // string code = await UserManager.GeneratePasswordResetTokenAsync(user.Identifiant);
                // var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Identifiant, code = code }, protocol: Request.Url.Scheme);		
                // await UserManager.SendEmailAsync(user.Identifiant, "Réinitialiser le mot de passe", "Réinitialisez votre mot de passe en cliquant <a href=\"" + callbackUrl + "\">ici</a>");
